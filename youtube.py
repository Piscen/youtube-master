...
#查询视频信息
# COMMAND_PREFIX_CHECK = 'python3 -m youtube-dl -F'

#下载1080p视频
# COMMAND_PREFIX_DOWNLOAD = 'youtube-dl -o "{}%(title)s.%(ext)s" -f 137+140 --newline --write-auto-sub '.format(DOWNLOAD_PATH)
COMMAND_PREFIX_DOWNLOAD = 'youtube-dl -o "{}%(title)s.%(ext)s" -f bestvideo+bestaudio --newline --write-auto-sub '.format(DOWNLOAD_PATH)
...
if __name__ == "__main__":
    ...
    urls=get_url()
    print(urls)
    executor = ThreadPoolExecutor(max_workers=THREAD_POOSIZE)
    for url in urls:
        executor.submit(dodownLoad,(url))